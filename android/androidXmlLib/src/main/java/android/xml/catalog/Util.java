...
class Util {
    ...
    static void validateFeatureInput(CatalogFeatures.Feature f, String value) {
        ...
        if (f == CatalogFeatures.Feature.PREFER) {
            if (!value.equals(CatalogFeatures.PREFER_SYSTEM) && !value.equals(CatalogFeatures.PREFER_PUBLIC)) {
                CatalogMessages.reportIAE(CatalogMessages.ERR_INVALID_ARGUMENT,
                        new Object[]{value, CatalogFeatures.Feature.PREFER.name()}, null);
            }
        } else if (f == CatalogFeatures.Feature.DEFER) {
            if (!value.equals(CatalogFeatures.DEFER_TRUE) && !value.equals(CatalogFeatures.DEFER_FALSE)) {
                CatalogMessages.reportIAE(CatalogMessages.ERR_INVALID_ARGUMENT,
                        new Object[]{value, CatalogFeatures.Feature.DEFER.name()}, null);
            }
        } else if (f == CatalogFeatures.Feature.RESOLVE) {
            if (!value.equals(CatalogFeatures.RESOLVE_STRICT) && !value.equals(CatalogFeatures.RESOLVE_CONTINUE)
                    && !value.equals(CatalogFeatures.RESOLVE_IGNORE)) {
                CatalogMessages.reportIAE(CatalogMessages.ERR_INVALID_ARGUMENT,
                        new Object[]{value, CatalogFeatures.Feature.RESOLVE.name()}, null);
            }

...
package android.com.sun.org.apache.xerces.internal.impl;

import android.com.sun.org.apache.xerces.internal.impl.io.ASCIIReader;
import android.com.sun.org.apache.xerces.internal.impl.io.UCSReader;
import android.com.sun.org.apache.xerces.internal.impl.io.UTF8Reader;
...
public class XMLEntityScanner implements XMLLocator  {
    ...
    protected String scanName(XMLScanner.NameType nt) throws IOException {
        ...
        String symbol;
        if (length > 0) {
            checkLimit(XMLSecurityManager.Limit.MAX_NAME_LIMIT, fCurrentEntity, offset, length);
        ...
        if (XMLChar.isNameStart(fCurrentEntity.ch[offset])) {
            ...
            while ( true){
                ...
                if (c == ':') {
                    ...
                    checkLimit(XMLSecurityManager.Limit.MAX_NAME_LIMIT, fCurrentEntity, offset, index - offset);
                }
            ...
            if (length > 0) {
                ...
                if (index != -1) {
                    ...
                    checkLimit(XMLSecurityManager.Limit.MAX_NAME_LIMIT, fCurrentEntity, offset, prefixLength);
                    ...
                    checkLimit(XMLSecurityManager.Limit.MAX_NAME_LIMIT, fCurrentEntity, index + 1, len);
     ...
     */
    protected void checkEntityLimit(XMLScanner.NameType nt, ScannedEntity entity, int offset, int length) {
     ...
     * @param length The length of the entity scanned
     */
    protected void checkLimit(XMLSecurityManager.Limit limit, ScannedEntity entity, int offset, int length) {
        fLimitAnalyzer.addValue(limit, entity.name, length);
        if (fSecurityManager.isOverLimit(limit, fLimitAnalyzer)) {
            fSecurityManager.debugPrint(fLimitAnalyzer);
            Object[] e = (limit == XMLSecurityManager.Limit.ENTITY_REPLACEMENT_LIMIT) ?
    ...
    protected int scanContent(XMLString content) throws IOException {
        ...
        // normalize newlines
        if (normalizeNewlines(Constants.XML_VERSION_1_0, content, false, false, null)) {
            return -1;
        }
    ...
    throws IOException {
        ...
        if (normalizeNewlines(Constants.XML_VERSION_1_0, content, false, true, null)) {
            return -1;
        }
    ...
    throws IOException {
        ...
        do {
            ...
            if (fCurrentEntity.position > fCurrentEntity.count - delimLen) {
                ...
                checkEntityLimit(XMLScanner.NameType.COMMENT, fCurrentEntity, fCurrentEntity.position, length);
            ...
            if (normalizeNewlines(Constants.XML_VERSION_1_0, buffer, true, false, XMLScanner.NameType.COMMENT)) {
                return true;
            }
            ...
            OUTER: while (fCurrentEntity.position < fCurrentEntity.count) {
                ...
                if (c == charAt0) {
                    ...
                    int length = fCurrentEntity.position - offset;
                    fCurrentEntity.columnNumber += length - newlines;
                    checkEntityLimit(XMLScanner.NameType.COMMENT, fCurrentEntity, offset, length);
    ...
    protected boolean normalizeNewlines(short version, XMLString buffer, boolean append,
            boolean storeWS, XMLScanner.NameType nt)
            throws IOException {
        // normalize newlines
        offset = fCurrentEntity.position;

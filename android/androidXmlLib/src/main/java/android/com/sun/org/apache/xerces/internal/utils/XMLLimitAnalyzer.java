...
package android.com.sun.org.apache.xerces.internal.utils;
...
public final class XMLLimitAnalyzer {
    ...
    public void addValue(int index, String entityName, int value) {
        if (index == XMLSecurityManager.Limit.ENTITY_EXPANSION_LIMIT.ordinal() ||
                ...
                index == XMLSecurityManager.Limit.TOTAL_ENTITY_SIZE_LIMIT.ordinal() ||
                index == XMLSecurityManager.Limit.ENTITY_REPLACEMENT_LIMIT.ordinal()
                ) {
            totalValue[index] += value;
            return;
        }
        if (index == XMLSecurityManager.Limit.MAX_ELEMENT_DEPTH_LIMIT.ordinal() ||
                index == XMLSecurityManager.Limit.MAX_NAME_LIMIT.ordinal()) {
            values[index] = value;
            totalValue[index] = value;
            return;
        }

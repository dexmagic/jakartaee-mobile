...
public final class XMLSecurityManager {
    ...
    public XMLSecurityManager(boolean secureProcessing) {
        ...
        states = new JdkProperty.State[Limit.values().length];
        isSet = new boolean[Limit.values().length];
        this.secureProcessing = secureProcessing;
        for (Limit limit : Limit.values()) {
            if (secureProcessing) {
                values[limit.ordinal()] = limit.secureValue;
                states[limit.ordinal()] = JdkProperty.State.FSP;
            } else {
                values[limit.ordinal()] = limit.defaultValue();
                states[limit.ordinal()] = JdkProperty.State.DEFAULT;
            }
        }
    ...
    public boolean setLimit(String propertyName, JdkProperty.State state, Object value) {
        ...
        if (index > -1) {
            ...
            if (index != indexEntityCountInfo && state == JdkProperty.State.APIPROPERTY) {
    ...
    private boolean getSystemProperty(Limit limit, String sysPropertyName) {
        try {
            ...
            if (value != null && !value.equals("")) {
                ...
                states[limit.ordinal()] = JdkProperty.State.SYSTEMPROPERTY;
                return true;
            }
            ...
            if (value != null && !value.equals("")) {
                values[limit.ordinal()] = Integer.parseInt(value);
                states[limit.ordinal()] = JdkProperty.State.JAXPDOTPROPERTIES;
                return true;
            }
